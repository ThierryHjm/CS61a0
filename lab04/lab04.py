...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    distance1 = abs(get_lat(city1) - get_lat(city2))
    distance2 = abs(get_lon(city1) - get_lon(city2))
    real_distance = sqrt(distance1**2 + distance2**2)
    print(real_distance)
...
def closer_city(lat, lon, city1, city2):
    ...
    dis1 = sqrt((lat1 - lat)**2 + (lon1 - lon)**2)
    dis2 = sqrt((lat2 - lat) ** 2 + (lon2 - lon) ** 2)
    if dis1 < dis2:
